...
class ObjectRenderer:
    def __init__(self, object: Object, color: np.ndarray = np.array([0.0, 0.5, 0.3]), line_color: np.ndarray = np.array([0.0, 0.1, 0.0])):
        # Set object locally
        self.object = object
    ...
    def update_vbos(self):
        ...
        self.vertices = np.array(self.object.mesh.vertices, dtype="f")
        self.vertex_buffer = vbo.VBO(self.vertices)

        self.vertex_normals = np.array(self.object.mesh.vertex_normals, dtype="f")
        self.vertex_normal_buffer = vbo.VBO(self.vertex_normals)

        self.faces = np.array(self.object.mesh.faces, dtype=np.int32)
        ...
        for i, face in enumerate(self.faces):
            face_normal = self.object.mesh.face_normals[i]
